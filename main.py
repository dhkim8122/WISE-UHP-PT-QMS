...
import { 
  ...
  Printer,
  AlertCircle,
  Percent // Added missing Percent icon import
} from 'lucide-react';
...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'uhp-pt-qms-final-v68';
...
const Dashboard = ({ data }) => {
  ...
  const downloadPdf = async () => {
    if (!window.html2canvas || !window.jspdf) {
        const s1 = document.createElement('script'); s1.src = "https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js";
        const s2 = document.createElement('script'); s2.src = "https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js";
        document.body.appendChild(s1); document.body.appendChild(s2);
        alert("라이브러리를 불러오고 있습니다. 잠시 후 다시 시도하세요.");
        return;
    }
...
// --- Component: Data List (Excel/Backup/Restore) ---
...
export default function App() {
  ...
  useEffect(() => {
    if (!user) return;
    const q = collection(db, 'artifacts', appId, 'public', 'data', 'defects');
  ...
  const addDefect = async (data) => {
    if (!user) return;
    await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'defects'), {
      ...data,
